...
?>
      <div id="3" class="task relative flex flex-col items-start p-4 mt-3 bg-white rounded-lg cursor-pointer bg-opacity-90 group hover:bg-opacity-100" draggable="true">
         <div class="absolute top-0 right-0 text-gray-500 flex gap-2 pt-1 pr-1">
            ...
            <a href="controllers/delete_task.php?id=<?php echo $doneTask['id'] ?>" class="delete-task-btn  active:scale-90" onclick="return confirm('Vouler vous vraiment supprimer cette tache')">
               <i class="fa-solid fa-xmark"></i>
            </a>
         </div>
         <span class="flex items-center h-6 px-3 text-xs font-semibold text-pink-500 bg-pink-100 rounded-full"><?php echo $doneTask['type'] ?></span>
         <h4 class="mt-3 text-sm font-medium"><?php echo $doneTask['titre'] ?></h4>
         ...
         <a href="controllers/changeStatut.php?id=<?php echo $doneTask['id']; ?>" class="mt-3 w-full text-blue-500 px-3 py-1.5 rounded-md flex justify-center items-center">
