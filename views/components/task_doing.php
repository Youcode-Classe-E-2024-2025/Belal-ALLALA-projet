<?php foreach($doingTasks as $doingTask){ ?>
   ...
   <div id="task" class="task relative flex flex-col items-start p-1 mt-3 bg-white rounded-lg cursor-pointer bg-opacity-90 group hover:bg-opacity-100 " draggable="true">
      <div class="absolute top-0 right-0 text-gray-500 flex gap-2 pt-1 pr-1 items-center ">
         ...
         <a href="controllers/delete_task.php?id=<?php echo $doingTask['id'] ?>" class="delete-task-btn active:scale-90" onclick="return confirm('Voulez-vous vraiment supprimer cette tâche?')">
            <i class="fa-solid fa-xmark"></i>
         </a>

      </div>

      <input type="hidden" name="task_id" value="<?php echo $doingTask['id'] ?>" >
      <span class="flex items-center h-6 px-2 text-xs font-semibold text-pink-500 bg-pink-100 rounded-full"><?php echo $doingTask['type'] ?></span>
      <h4 class=" text-sm font-medium"><?php echo $doingTask['titre'] ?></h4>
      <div class="flex w-full items-center ">
         ...
         <a href="controllers/changeStatut.php?id=<?php echo $doingTask['id']; ?>" class="w-full text-blue-500 px-3 py-1.5 rounded-md flex justify-center items-center ">
   ...
   <div id="hiddenSection<?php echo $doingTask['id'] ?>" class="section-anim overlay hidden w-screen h-screen fixed top-0 left-0 z-20  flex items-center ">
      <div class="p-6 rounded-lg shadow-md bg-white w-1/2 h-full">

         <div class="flex items-center w-full justify-between">
            <h2 class="text-2xl font-bold mb-4 text-center mr-auto ml-auto"><?php echo $doingTask['titre'] ?></h2>
         ...
         <div class="space-y-6">
            ...
            <div class="flex items-center space-x-6">
               ...
               <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 mr-2"><?php echo $doingTask['type'] ?></span>
            </div>
            ...
            <div class="flex items-center space-x-6">
               ...
               <?php 
                  ...
                  $category=$categoryTaskModel->getCategoriesByTask($doingTask['id']);
            ...
            <div>
               ...
               <p class=" border p-4 rounded-lg bg-gray-50 text-start">
                  <?php echo $doingTask['description'] ?>
               </p>
            </div>
